...
select 
  count(visitor_id) as num_of_visitors 
from 
  sessions;
...
last_paid_click as (
  ...
  from 
    sessions as s 
    inner join last_visits as lv on s.visitor_id = lv.visitor_id 
    and s.visit_date = lv.last_visit 
...
select 
  ...
  sum(
    case when closing_reason = 'успешно реализовано' 
    or status_id = 142 then 1 else 0 end
  ) as purchases_count 
from 
  last_paid_click;
...
select 
  round(
    leads_count * 100.00 / visitors_count, 
    2
  ) as visit_to_lead, 
  round(
    purchases_count * 100.00 / leads_count, 
    2
  ) as lead_to_purchase 
from 
  visit_lead_purchase;
...
from 
  cost as c 
  inner join revenue as r on c.source = r.source;
-- основные метрики 
with last_visits as (
  select 
    s.visitor_id, 
...
aggregate_last_paid_click as (
  ...
  from 
    aggregation as a 
    left join ad_costs as ac on a.visit_date = ac.campaign_date :: date 
...
select 
  ...
  case when sum(visitors_count) > 0 then round(
    sum(total_cost) / sum(visitors_count), 
    2
  ) else 0 end as cpu, 
  case when sum(leads_count) > 0 then round(
    sum(total_cost) / sum(leads_count), 
    2
  ) else 0 end as cpl, 
  case when sum(purchases_count) > 0 then round(
    sum(total_cost) / sum(purchases_count), 
    2
  ) else 0 end as cppu, 
  case when sum(total_cost) > 0 then round(
    (
      sum(revenue) - sum(total_cost)
    ) * 100.0 / sum(total_cost), 
    2
  ) else 0 end as roi 
from 
